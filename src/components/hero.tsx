...
const HERO_PHRASE_KEYS = [
  "home.hero.phraseHistory",
  "home.hero.phrasePhotos",
  "home.hero.phraseMoments",
] as const;

export function Hero() {
  const { t } = useTranslation();
  const [modalOpen, setModalOpen] = useState(false);
  const [phraseIndex, setPhraseIndex] = useState(() => Math.floor(Math.random() * HERO_PHRASE_KEYS.length));

  useEffect(() => {
    const interval = setInterval(() => {
      setPhraseIndex((current) => (current + 1) % HERO_PHRASE_KEYS.length);
    }, 10000); // 10 seconds
    ...
    <section className="relative w-full overflow-hidden bg-[#171717]">
      ...
      <div className="relative z-10 mx-auto flex min-h-[calc(100vh-76px)] max-w-7xl items-center">
        <div className="ml-auto flex w-full flex-col justify-center gap-6 px-8 py-16 md:w-[60%] md:py-24 lg:w-[55%] lg:px-16">
          ...
          >
            {t(HERO_PHRASE_KEYS[phraseIndex])}
          </h1>
